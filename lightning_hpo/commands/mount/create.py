...
class MountConfig(SQLModel, table=True):
...
class CreateMountCommand(ClientCommand):
    ...
    def run(self) -> None:
        ...
        parser.add_argument("--source", type=str, required=True, help="The associated S3 URL of the Mount.")
        parser.add_argument(
            "--mount_path", type=str, default=None, help="Where the Mounts should be mounted to the works"
        )

        hparams = parser.parse_args()
        mount_path = hparams.mount_path if hparams.mount_path else os.path.join("./mount", hparams.name)
        response = self.invoke_handler(
            config=MountConfig(name=hparams.name, source=hparams.source, mount_path=mount_path)
        )
        print(response)
